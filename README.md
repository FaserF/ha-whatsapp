...
- **✈️ Native WhatsApp <-> Telegram Bridge**: Bi-directional message mirroring, 1:1 Direct Chat Mirroring (connecting single-contact WhatsApp groups cleanly with standalone Telegram Bots without header clutter), automatic Telegram chat discovery via Telegram Bot API, media attachment sync, configurable metadata headers, and intelligent thread quote resolution across WhatsApp and Telegram.
