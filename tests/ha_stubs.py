...
def _get_or_create_class(name: str, base: type = object) -> type:
    ...
    if not hasattr(sys, attr):
        # Create the class
        if base == Exception:
            class StubException(Exception): pass
            cls = StubException
        else:
            class StubClass(base): pass
            cls = StubClass
...
# Core
class ServiceCall:
    def __init__(self, domain: str, service: str, data: dict[str, Any] | None = None, **_kwargs: Any) -> None:
        self.domain = domain
...
class ConfigFlow:
    def __init_subclass__(cls, **_kwargs: Any) -> None: pass
    def __init__(self) -> None:
        self.hass: Any = None
...
class RepairsFlow(ConfigFlow): pass
...
def mock_add_entities(hass: Any, entities: list[Any], update_before_add: bool = False) -> None:
...
class MockConfigEntry:
    def __init__(self, domain: str, data: dict[str, Any], title: str = "WhatsApp", entry_id: str | None = None, options: dict[str, Any] | None = None, **kwargs: Any) -> None:
        self.domain = domain
        self.data = data or {}
        if entry_id is None:
            import uuid
    ...
    def async_on_unload(self, func: Callable[..., Any]) -> None: pass
    def add_update_listener(self, func: Callable[..., Any]) -> Callable[[], None]: return lambda: None
    def as_dict(self) -> dict[str, Any]:
        return {
            "entry_id": self.entry_id, "domain": self.domain, "title": self.title,
            "data": self.data, "options": self.options, "unique_id": self.unique_id, "version": self.version,
        }

    def add_to_hass(self, hass: Any) -> None:
        if "entries" not in hass.data: hass.data["entries"] = {}
        hass.data["entries"][self.entry_id] = self
        if "whatsapp" not in hass.data: hass.data["whatsapp"] = {}
...
def _build_ha_stub_modules() -> None:
    ...
    # homeassistant.core
    _stub("homeassistant.core", HomeAssistant=object, callback=lambda f: f, ServiceCall=ServiceCall, Bus=Bus)

    # homeassistant.const
    _stub("homeassistant.const", CONF_URL="url", CONF_API_KEY="api_key", Platform=Platform, EntityCategory=EntityCategory)
    ...
    cv_mod = _stub("homeassistant.helpers.config_validation", string=str, ensure_list=list, boolean=bool, match_all=lambda x: x)

    # homeassistant.helpers.entity_registry
    _stub("homeassistant.helpers.entity_registry", async_get=MagicMock(), async_entries_for_config_entry=MagicMock())

    # homeassistant.helpers.issue_registry
    ir_mod = _stub("homeassistant.helpers.issue_registry", async_get=MagicMock(), IssueSeverity=MagicMock(), async_delete_issue=MagicMock(), async_create_issue=MagicMock())
    ...
    _stub("homeassistant.data_entry_flow", FlowResultType=FlowResultType, AbortFlow=Exception, FlowResult=dict)

    # homeassistant.config_entries
    _stub("homeassistant.config_entries", ConfigEntry=object, ConfigFlow=ConfigFlow, OptionsFlow=OptionsFlow, SOURCE_USER="user", SOURCE_REAUTH="reauth")
    ...
    # components.repairs
    repairs_mod = _stub("homeassistant.components.repairs", RepairsFlow=RepairsFlow, ConfirmRepairFlow=ConfirmRepairFlow)
    components.repairs = repairs_mod

    # homeassistant.util
    util_mod = _stub("homeassistant.util", slugify=lambda x: x.lower().replace(" ", "_"))
    import datetime
    dt_mod = _stub("homeassistant.util.dt", utcnow=lambda: datetime.datetime.now(datetime.UTC), now=lambda: datetime.datetime.now(), as_local=lambda dt: dt)
    util_mod.dt = dt_mod
